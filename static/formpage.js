...
form.addEventListener('submit', function (evt) {
    ...
    for (let field of fields) {
        if (field.value === '') { ok = false; field.classList.remove('green'); field.classList.add('red'); 
        validation_field.classList.remove('form_validation_field'); validation_field.innerHTML = error_when_fields_are_empty };
    ...
    if (ok === true) {
        validation_field.classList.add('form_validation_field'); button.disabled = true; for (let field of fields) { field.classList.remove('green') }; fetch('/register', {
            ...
            .then(response => {
                button.disabled = false; for (let field of fields) { field.classList.remove('green'); field.value = '' };
